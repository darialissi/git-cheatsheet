...
- [Изменения в коммитах](#изменения-в-коммитах)
  ...
  - ["Редактирование" коммита](#редактирование-коммита)
...
**git config --global core.editor "gedit -s"** - настроить редактор(gedit) по умолчанию
...
3. **git commit** - добавление комментария через текстовый редактор
    ...
    - **git commit --amend** - изменение коммита (viz "Изменения в коммитах")
...
**git checkout -- [file name]** - отменить все локальные незакоммиченные изменения конкретного файла
...
1. вариант
	1. **git reset --soft [номер коммита - HEAD по умолчанию]** - передвигает указатель, но оставляет текущие рабочую директорию и индекс
	2. **git commit -с ORIG_HEAD** - новый коммит, все последующие становятся недосягаемыми
2. вариант - только для последнего коммита
	1. **git commit --amend**
